...
class Network(object):
    ...
    def SGD(self, training_data, epochs, mini_batch_size,
            ...
            early_stopping_n=0, constant_eta=True,
            eta_shrinkage_times=10, eta_decrease_factor=0.0001):
        ...
        #        0.5*lmbda*l2_norm_squared/size(training_data)
        cost = self.layers[-1].cost(self) + \
               0.5*lmbda*l2_norm_squared/num_training_batches
        grads = T.grad(cost, self.params)
        shared_eta = theano.shared(eta,borrow=True) # (same as shared_b) use SharedVariable instead of value
        updates = [(param, param - T.cast(shared_eta*grad, dtype=theano.config.floatX))
        ...
        for epoch in range(epochs):
            ...
            #============================================================================================
            # From https://kezunlin.me/post/566c93e7
            # Early stopping with variable learning rate
            # (1) no_accuracy_change >= early_stopping_n: stop          NIN = 20
            # (2) new_eta = 1/2*eta until new_eta<=1/1024*eta           ETA_SHRINKAGE_TIME = 10 
            #============================================================================================

            # (1) Check in last epoch of NIN stage
            if early_stopping_n > 0:
                ...
                if (no_accuracy_change >= early_stopping_n):
                    ...
                    print('[HIT] Early stopping at epoch #{0}, iteration #{1}, validation accuracy {2:.2%}'.format(
                        epoch, best_iteration, best_validation_accuracy))
                    break
            
            #******************************************************************************
            if ((epoch + 1) % (early_stopping_n / 2) == 0) and (not constant_eta):
                # (2) shrinkage eta (accuracy not improved too much)
                cur_eta_shrinkage_times += 1
                print('cur_eta_shrinkage_times = {0}'.format(cur_eta_shrinkage_times))
                if cur_eta_shrinkage_times > eta_shrinkage_times:
                    print('[HIT] Eta shrinkage OK. at epoch #{0}, iteration #{1}, validation accuracy {2:.2%}'.format(
                        epoch, best_iteration, best_validation_accuracy))
                    break
                
                # Update eta every epoch
                # new_eta = eta/(2.0**cur_eta_shrinkage_times)
                new_eta = eta/(1.0 + eta_decrease_factor*(epoch + 1))
    ...
    def save_network(self, best_model):
