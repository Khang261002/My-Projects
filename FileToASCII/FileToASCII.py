...
def generateFrameConversionData(imgPath, type):
    ...
    for y in range (0, size[1]):
        for x in range (0, size[0]):
            ...
            if type == 1:
                ...
                asciiChar = greyscaleCharsRev[str(pix)]
...
if __name__ == "__main__":
    ...
    srcFile = input("Enter name of source file with directory:\t")
    dstFile = input("Enter name for new ascii file:\t\t\t")
    type = int(input("Choose type of color to generate\n\t1. Default\n\t2. Reverse\nEnter type you want:\t\t\t\t"))

    __srcFile_location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(srcFile)))
    __dstFile_location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(dstFile)))
    __current_location__ = os.path.realpath(os.path.join(os.getcwd(), os.path.dirname(__file__)))

    if __srcFile_location__ != __current_location__:
        srcFile_name = srcFile.replace(__srcFile_location__ + "\\", '')
        srcFile_copy = __current_location__ + "\\" + srcFile_name
        shutil.copyfile(srcFile, srcFile_copy)
        print(srcFile_name + " created in " + __current_location__)
        srcFile = srcFile_name
    
    elif __srcFile_location__ in srcFile:
        srcFile = srcFile.replace(__srcFile_location__ + "\\", '')

    if check_type(srcFile) == "image":
        generateFrameConversionData(srcFile, type)
        ...
        for frame in srcFrames:
            vidContent.append(generateFrameConversionData(frame, type))
    ...
    if __srcFile_location__ != __current_location__:
        os.remove(srcFile_copy)
        if __dstFile_location__ not in dstFile:
            dstFile_copy = __srcFile_location__ + "\\" + dstFile
            shutil.copy(dstFile, dstFile_copy)
            os.remove(dstFile)
